...
**disabled by default** (`VULNBOT_MCP=0`) and stays a no-op until a specific server + version is
...
Gating, so the system is safe with MCP absent:
  - `VULNBOT_MCP` unset / "0" / falsey  → `is_enabled()` is False → `supports()` returns False for
    every action → the router (TL-1.4) never routes here → pure no-op. SSH + msfrpc do everything.
  - `VULNBOT_MCP` truthy → the channel offers itself only for actions that name an MCP tool, and
    `run()` FIRST verifies the configured server + version (`VULNBOT_MCP_SERVER` /
    `VULNBOT_MCP_VERSION`). Verification failure → `ChannelError` → the Executor falls back to a
...
class McpChannel(Channel):
    """Flag-gated MCP tool bridge. Disabled unless `VULNBOT_MCP` is truthy AND a server/version is
    ...
    Injection points for tests / a future real transport:
      - `verifier()` → truthy when the configured MCP server + version are present and acceptable.
        Default: both `VULNBOT_MCP_SERVER` and `VULNBOT_MCP_VERSION` are set (a placeholder check
    ...
    def is_enabled() -> bool:
        ...
        return _env_truthy("VULNBOT_MCP")
    ...
    def _verify(self) -> None:
        ...
        ok = self._verifier() if self._verifier is not None else (
            bool(os.environ.get("VULNBOT_MCP_SERVER")) and bool(os.environ.get("VULNBOT_MCP_VERSION"))
        )
        if not ok:
            raise ChannelError(
                "mcp: server/version not verified "
                "(set VULNBOT_MCP_SERVER + VULNBOT_MCP_VERSION, or inject a verifier)"
            )
    ...
    def run(self, action: Action, action_id: str) -> Observation:
        if not self.is_enabled():
            ...
            return Observation.failure(
                ...
                error="mcp: channel disabled (VULNBOT_MCP=0)",
