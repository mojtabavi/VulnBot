...
## Corpus Check
- 14 files · ~120,224 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1359 nodes · 2728 edges · 99 communities (92 shown, 7 thin omitted)
...
Cohesion: 0.12
Nodes (25): _belief_with_vuln(), _Ctrl, _exploit_cands(), _Fx, _mk(), TL-2.6 — R1 belief-loop guarantees: self-consistency (Z averaging + variance red, Scripted control server: `awaits` feeds blocking recv, `polls` feeds the non-blo, A Z-likelihood JSON with the given P(observation|present); absent is the complem (+17 more)
...
Cohesion: 0.12
Nodes (27): Action, _action_utility(), ActionType, add_host(), _default_dist(), _entropy(), _get_dist(), new_host_prior() (+19 more)
...
Cohesion: 0.11
Nodes (16): JSONLoader, file_exists_in_db(), TextSplitter, files2docs_in_thread(), files2docs_in_thread_file2docs(), get_loader(), get_LoaderClass(), get_vs_path() (+8 more)

### Community 14 - "Community 14"
Cohesion: 0.12
Nodes (15): get_kb_details(), get_kb_file_details(), Config, KnowledgeBaseModel, KnowledgeBaseSchema, delete_kb_from_db(), kb_exists(), list_kbs_from_db() (+7 more)

### Community 15 - "Community 15"
Cohesion: 0.11
Nodes (18): commandMeta(), CommandResult, COMMANDS, CommandSpec, fetchingLabel(), handleCommand(), getModel(), getModelConfig() (+10 more)
...
Cohesion: 0.17
Nodes (22): App(), DbSettings, DEFAULT_PREFS, dumpYaml(), getAuthMode(), getKali(), getProviderId(), here (+14 more)
...
Cohesion: 0.11
Nodes (18): _anthropic_has_thinking(), _downgrade_thinking(), _effective_thinking(), _is_retryable(), _notify_llm_ok(), _notify_llm_wait(), The configured level, capped by any downgrade forced by a prior rate limit., Lower the process-wide cap to one step below `current` (floor 'off') after a 429 (+10 more)
...
Cohesion: 0.12
Nodes (10): TL-0.3 — the loopback control channel round-trips a frame each direction (R2 tra, test_control_frame_roundtrip_both_directions(), test_no_client_is_non_fatal(), ControlClient, ControlServer, Send one event frame to the CLI (agent → CLI). False if no client / send failed., Receive one command frame from the CLI (CLI → agent). None if no client / timeou, Test/utility client (the real front-end is `cli/src/control.ts`). Connects to a (+2 more)
...
Cohesion: 0.23
Nodes (14): init(), Event, FastAPI, create_app(), main(), run_api_server(), run_webui(), _set_app_event() (+6 more)

### Community 37 - "Community 37"
Cohesion: 0.15
Nodes (16): DEFAULT_DB, ExecutorMode, getDbConfig(), listProviders(), MysqlMode, savePrefs(), firstLlmStep(), getProvider() (+8 more)
...
Cohesion: 0.18
Nodes (15): ModelSettings, fetchModels(), FetchModelsOpts, headersFor(), parseModels(), AuthMode, BY_ID, listProviders() (+7 more)

### Community 40 - "Community 40"
Cohesion: 0.24
Nodes (15): COMPOSE_BASE, dbReachable(), docker(), dockerAvailable(), ensureDb(), ensureKali(), initDb(), labDown() (+7 more)

### Community 41 - "Community 41"
Cohesion: 0.14
Nodes (13): ApproveFn, BeliefLLM, GoalFn, _default_goal(), BeliefAgent — the standalone POMDP control loop (R1, TL-2.1).  This is the integ, # NOTE: `action_type` (not `type`) — `type` is EventLog.append's positional para, Z self-consistency sample count. Explicit arg wins; else the shared     `belief_, Default stop predicate: a host is believed rooted, or the last observation clear (+5 more)

### Community 42 - "Community 42"
Cohesion: 0.18
Nodes (10): add_docs_to_db(), add_file_to_db(), count_files_from_db(), delete_docs_from_db(), delete_file_from_db(), delete_files_from_db(), list_docs_from_db(), 列出某知识库某文件对应的所有Document。     返回形式：[{"id": str, "metadata": dict}, ...] (+2 more)

### Community 43 - "Community 43"
Cohesion: 0.19
Nodes (8): socket, _FramedConn, Loopback control channel — the CLI↔agent back-channel for human-in-the-loop (R2), Newline-delimited JSON framing over one connected socket (shared send/recv logic, Block until the CLI connects (or `timeout`). Returns True once connected., _clean(), emit(), Structured run-progress markers for the octopus CLI.  The pentest pipeline strea

### Community 44 - "Community 44"
Cohesion: 0.21
Nodes (8): BaseFileSettings, ConfigsContainer, DBConfig, KBConfig, LLMConfig, Enum, Config, replace_ip_with_targetip()
...
Cohesion: 0.25
Nodes (5): ABC, OllamaChat, OpenAIChat, StrEnum, LLMType
...
- **Why does `BeliefAgent` connect `Community 6` to `Community 0`, `Community 41`, `Community 8`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `KBService` connect `Community 12` to `Community 33`, `Community 42`, `Community 13`, `Community 14`, `Community 49`, `Community 24`?**
